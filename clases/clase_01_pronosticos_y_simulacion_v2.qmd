---
listing_title: "Clase 01 v2"
title: |
  | \includegraphics[width=5cm]{Imagen5.png}  
  | ECO5008 Modelos predictivos
subtitle: Clase 01 v2 - Pronosticos y Simulacion en la Gestion Hospitalaria
format: pdf
editor: visual
author:   
...
Introducir conceptos básicos de series de tiempo y técnicas iniciales de pronóstico en R para apoyar decisiones operativas en instituciones de salud.

## Problema de gestión que aborda

Las instituciones de salud enfrentan incertidumbre sobre la demanda futura de atenciones, lo que dificulta la planificación de recursos clínicos, capacidad operativa y asignación de turnos.

## Decisión que apoya el modelo

El modelo apoya decisiones de planificación anticipada, tales como estimar demanda futura, asignar recursos de manera preventiva y preparar respuestas frente a cambios estacionales o aumentos de presión asistencial.

## Aprendizajes esperados

Al finalizar la clase, el estudiante debería ser capaz de identificar los componentes de una serie de tiempo, aplicar técnicas básicas de pronóstico en R e interpretar sus resultados desde una perspectiva de gestión.

## Aplicación en instituciones de salud

Este enfoque puede aplicarse al pronóstico de consultas, atenciones de urgencia, hospitalizaciones o demanda por causas estacionales, facilitando una mejor preparación operativa.

# Conceptos iniciales

## Conceptos básicos de series de tiempo

Se habla de series de tiempo cuando nos referimos a una variable sobre la cual observamos su comportamiento a través del tiempo. Un ejemplo conocido es el principal indicador de productividad de un país, denominado Producto Interno Bruto (PIB). A continuación, se presenta la siguiente imagen:
...
Considérese el caso del precio de la acción de LATAM Airlines `r fa("plane-departure", fill = "steelblue")`:

```{r, message=FALSE, warning=FALSE, echo=FALSE}
# install.packages(c("readxl", "tidyverse", "scales"))  # si no los tienes
library(readxl)
library(tidyverse)
library(scales)
...
stopifnot(!is.na(price_path))

desde  <- "2018-01-01"

# 1) Leer precios diarios desde archivo local
precios <- read_excel(price_path, sheet = "precios") %>%
  mutate(date = as.Date(date)) %>%
  filter(date >= as.Date(desde), date <= Sys.Date())
...
ggplot(precios, aes(date, adjusted)) +
  ...
  labs(
    ...
    x = "Fecha", y = "Precio ajustado (CLP)", caption = "Fuente: archivo local precios_latam_ltm_sn.xlsx"
  ) +
  theme_minimal()
...
# 1) Filtrar la serie desde el archivo local
precios <- read_excel(price_path, sheet = "precios") %>%
  mutate(date = as.Date(date)) %>%
  filter(date >= as.Date(desde), date <= as.Date(hasta))
...
¿Qué ocurre si se filtra la serie desde 2024 hasta la fecha?
...
- Estacionalidad: corresponde a variaciones de la serie que poseen un comportamiento predecible ya sea semanal, mensual o anual.
...
Se debe justificar por qué esto es necesario en el contexto de negocios. En términos generales, el interés científico sobre una serie puede darse por dos razones:

1.  Explicar (por qué)
2.  Predecir (qué y cuánto)

A pesar de esto, el límite entre ambas dinámicas es muy delgado. Considérese el siguiente ejemplo:
...
En términos generales, este modelo se utiliza para explicar factores que influyen en la renta de las personas.

Pero ¿qué ocurre si, en vez de explicar la variable, asumimos que el modelo también puede predecir la variable de interés? `r fa("arrow-up-right-dots", fill = "steelblue")`

- Pensemos en el comportamiento tradicional que se hace en torno a los promedios de una variable.

## Predecir, pronosticar y simular

En términos generales, predecir se relaciona con contextos más amplios que solo estimar el valor futuro de una serie temporal. Puede referirse a estimar el valor de una variable desconocida en distintos contextos temporales, como también a variables no temporales, por ejemplo, en tareas de clasificación.

En el caso de pronosticar, este concepto se relaciona con estimar el valor futuro de una variable en una serie de tiempo. En este sentido, el pronóstico refiere a un contexto más acotado dentro de la dinámica predictiva. **En este sentido, todo pronóstico es una predicción, pero no toda predicción es un pronóstico**.
...
Considérese la situación de negocios más simple. Calculamos el promedio y este se convierte en nuestro pronóstico:
...
Somos un equipo de analistas y debemos estimar una manera razonable de anticipar cuántas atenciones tendremos en el futuro por causas respiratorias. Utilizaremos el siguiente dataset del Gobierno de Chile:
...
Para esto generaremos un resumen general mas liviano del dataset, evitando dependencias adicionales y manteniendo solo las metricas principales.
...
En esta version, dejamos deshabilitado el reporte exploratorio extendido para mantener la clase mas liviana:
...
Con esto podemos darnos una primera impresion del archivo ya filtrado y de algunas caracteristicas principales de sus variables.

Nos centraremos en el total de atenciones para un centro de atencion en particular; para esto generamos los siguientes filtros:
...
1.  ¿Qué conclusión puede sacar del promedio móvil? ¿Y del suavizamiento?
...
En base a la serie ya analizada, definimos una simulación de Monte Carlo en base a lo siguiente:
...
Para este caso, el principal valor de las simulaciones viene dado por la posibilidad de seleccionar métodos con un mayor o menor grado de incertidumbre; por otra parte, se podría también trabajar en sensibilizar el posible impacto de la predicción ya obtenida en base a los valores relevantes de la simulación.

## 🏥 Implicancias para la gestión

Desde una perspectiva gerencial, este tipo de análisis no solo permite generar un pronóstico puntual, sino también cuantificar la incertidumbre en torno a la demanda futura. Esto puede apoyar decisiones de planificación de turnos, asignación de personal, preparación de camas y coordinación de recursos en períodos de mayor presión asistencial.

En instituciones de salud, una proyección con bandas de incertidumbre resulta especialmente útil cuando la decisión no depende solo del valor esperado, sino también del riesgo asociado a escenarios altos de demanda.
