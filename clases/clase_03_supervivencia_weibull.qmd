...
subtitle: Clase 03 - Modelos de Supervivencia y Weibull aplicados a Salud
format: pdf
editor: visual
author:   
...
Introducir los fundamentos del análisis de supervivencia y el uso de la distribución de Weibull en R para modelar tiempos hasta evento en contextos de salud.

## Problema de gestión que aborda

En instituciones de salud, muchas decisiones dependen de estimar cuánto tiempo transcurre hasta un evento relevante, como el alta, la rehospitalización, una falla de equipo o la necesidad de mantenimiento.

## Decisión que apoya el modelo

El modelo apoya decisiones relacionadas con planificación clínica y operativa, tales como estimar duración de estadías, anticipar altas, programar mantenimiento y evaluar riesgos asociados a eventos futuros.

## Aprendizajes esperados

Al finalizar la clase, el estudiante debería ser capaz de interpretar datos con censura, ajustar un modelo de Weibull en R y traducir sus resultados en implicancias útiles para la gestión.

## Aplicación en instituciones de salud

Este enfoque puede aplicarse a tiempos de hospitalización, seguimiento de pacientes, análisis de rehospitalización, confiabilidad de equipamiento crítico y definición de estrategias de mantenimiento.

# Conceptos iniciales

## Introducción a supervivencia

Cuando hablamos de supervivencia en términos de predicciones nos referimos al esfuerzo de modelar el tiempo que transcurre hasta cierto evento. En el caso del área de salud, podemos pensar en el tiempo que transcurre hasta remisión, alta o rehospitalización. Un área intermedia de aplicación corresponde a la de ingeniería de la confiabilidad, sirviendo para poder determinar tiempo entre fallas o entre reparaciones.

## Elementos claves

-   La variable relevante para ser analizada corresponde al tiempo hasta el evento, considerando $T > 0$

-   Censura: corresponde a un evento que, por alguna u otra razón, no puede ser observado de manera completa; por ejemplo, si estamos analizando el tiempo antes del alta de los pacientes, existirán casos de pacientes que aún se encuentran hospitalizados, pero que, dado el momento de selección del estudio, debemos **censurar** en base al valor que poseen al momento del estudio. Este tipo de observación aporta información a la probabilidad de que el evento ocurra con posterioridad a lo observado y debe marcarse para incorporarla de manera distinta al análisis.
...
-   ¿Qué decisiones clínicas, operativas o de mantenimiento podrían beneficiarse de un modelo de tiempo hasta evento?
-   ¿En qué contextos de una institución de salud sería importante distinguir entre eventos observados y observaciones censuradas?
-   ¿Cómo podría este tipo de análisis apoyar la planificación de capacidad, seguimiento de pacientes o confiabilidad de equipamiento?

# Distribución de Weibull

Se utiliza para modelar los tiempos hasta un evento cuando la tasa de eventos cambia de forma monótona (crece o decrece).
...
\item $ \textbf{Riesgo instantáneo (hazard):}\quad h(t)=\dfrac{k}{\eta}\ \left(\frac{t}{\eta}\right)^{k-1} $
...
Tengamos los siguientes datos de altas de pacientes con neumonía y queremos predecir el tiempo promedio de hospitalización en días:
...
Recordar que, en este caso, la censura corresponde a los pacientes que no han sido dados de alta (alta = 0).
...
A continuación, se presenta el análisis de Weibull para estos datos:
...
download_with_retry <- function(url, destfile, mode = "wb", quiet = TRUE,
  ...
  for (i in seq_len(retries)) {
    ...
    if (ok && file.exists(destfile) && file.info(destfile)$size > 0) {
      message(sprintf("✅ Descarga OK en intento %d.", i))
      return(TRUE)
    } else {
      if (i < retries) {
        message(sprintf("⏳ Falla intento %d. Reintentando en %ds...", i, wait))
        Sys.sleep(wait)
...
if (download_ok) {
  ...
  message(sprintf("💾 Copia cacheada en: %s", normalizePath(local_path)))
} else if (file.exists(local_path)) {
  file_to_read <- local_path
  message(sprintf("📂 Usando copia local: %s", normalizePath(local_path)))
} else {
  stop("❌ No se pudo descargar el .xlsm y no existe copia local en 'data/'.")
}

# --- 3) Verificar que la hoja exista (xlsm puede tener varias hojas) ---
sheets <- readxl::excel_sheets(path = file_to_read)
if (!(sheet_name %in% sheets)) {
  stop(sprintf("❌ La hoja '%s' no existe. Hojas disponibles: %s",
...
Se sabe que las máquinas de hemodiálisis son necesarias para mantener el tratamiento de los pacientes, requiriendo siempre un funcionamiento eficiente y continuo. Utilizando la distribución de Weibull podemos caracterizar la dinámica de fallas para dichas máquinas.
...
## 🛠️ Intervalo óptimo de mantención

Considerando que ya podemos caracterizar la dinámica de fallas para cada máquina, ¿podríamos realizar algún análisis que permita definir una estrategia para evitar la falla de la maquinaria? Aquí debemos entender la diferencia entre falla correctiva y falla preventiva:
...
-   $\lambda(T)$: tasa de falla o riesgo instantáneo (hazard rate) al tiempo
